...
# Initialize speaker diarization with proper error handling
...
try:
    ...
    if not HF_TOKEN:
        logger.warning("⚠️ HUGGINGFACE_TOKEN not found in .env file")
    else:
    ...
    logger.error("⚠️ pyannote.audio not installed. Run: pip install pyannote.audio")
except Exception as e:
    logger.error(f"⚠️ Failed to load speaker diarization: {str(e)}")
    logger.error("Continuing WITHOUT speaker identification...")
...
def transcribe_audio_with_speakers(file_path):
    ...
    # Perform diarization first (if available)
