...
# UPDATED PATHS - Save in your specified directory
BASE_FOLDER = r"C:\Users\Jimbo\Documents\FE Python Transcription App"
AUDIO_FOLDER = os.path.join(BASE_FOLDER, "audio_files")  # Folder to store downloaded audio
ANALYSIS_FOLDER = os.path.join(BASE_FOLDER, "analysis_results") # Folder to store analysis results
TRANSCRIPTION_FOLDER = os.path.join(BASE_FOLDER, "transcriptions") # NEW: Folder for transcriptions
...
def save_transcription_to_file(transcript, base_filename, transcription_folder):
    """Saves the transcript content to a text file."""
    try:
        # Create filename like 'recording-public_4_transcript.txt'
        transcript_filename = f"{os.path.splitext(base_filename)[0]}_transcript.txt"
        transcript_filepath = os.path.join(transcription_folder, transcript_filename)
...
Please perform the following tasks for each provided recording URL:

1. **Transcription**:
   - Fully transcribe the audio from each recording URL.
   - Save each transcription in a separate folder with an appropriate name.

2. **Application Submission Information**:
   - For each recording, determine whether an application was submitted by identifying the following **Application Submission Signals**:
     - The customer provides **bank routing and account numbers** OR **credit card number**.

     **IF an application was submitted (YES):**
     - Provide the following details:
       a. Carrier Name
       b. Monthly Premium
       c. Customer's Age

     **IF an application was not submitted (NO):**
     - Explain **why** the application was not submitted.

3. **Agent's Questions**:
   - List **all** questions that the insurance agent asked the customer during the call in the exact order they were asked.  
   - **Do not skip or omit any question**.

4. **Agent's Greeting**:
   - Provide the exact **greeting statement** the agent used at the beginning of each call.

### Formatting Requirements:
- Organize the output clearly, using numbered lists or bullet points for each section.
- Ensure the response is concise but includes all requested details.
- Save transcriptions in appropriately named folders for easy identification.
"""
        messages = [
...
def process_single_call(url, audio_folder, analysis_folder, transcription_folder):
    """Downloads, transcribes, analyzes, saves analysis, saves transcription, and optionally cleans up audio."""
    ...
    try:
        ...
        if not audio_file_path:
            ...
            if not transcript:
                ...
                # NEW: Save transcription to file
                if save_transcription_to_file(transcript, audio_filename, transcription_folder):
                    logger.info(f"Transcription saved for {audio_filename}")
                else:
                    logger.error(f"Failed to save transcription for {audio_filename}")
        ...
        # 5. OPTIONAL: Clean up downloaded audio file (COMMENT OUT IF YOU WANT TO KEEP AUDIO FILES)
        ...
        # Instead, log where audio file is saved:
        if audio_file_path and os.path.exists(audio_file_path):
...
def process_all_calls(url_file='urls3.txt', audio_folder=AUDIO_FOLDER, analysis_folder=ANALYSIS_FOLDER, transcription_folder=TRANSCRIPTION_FOLDER):
    ...
    logger.info(f"Files will be saved in: {os.path.abspath(BASE_FOLDER)}")
    logger.info(f"- Audio files: {os.path.abspath(audio_folder)}")
    logger.info(f"- Transcriptions: {os.path.abspath(transcription_folder)}")
    logger.info(f"- Analysis results: {os.path.abspath(analysis_folder)}")

    # FIXED: Initialize status_counts dictionary
    status_counts = {
        "Analysis Success": 0,
        "Download Failed": 0,
    ...
    for i, url in enumerate(urls, 1):
        ...
        try:
             # Pass all folder paths to the processing function
             final_status = process_single_call(url, audio_folder, analysis_folder, transcription_folder)
    ...
    logger.info(f"Files saved in: {os.path.abspath(BASE_FOLDER)}")
    logger.info("=" * 70)
...
if __name__ == "__main__":
    ...
    # Ensure all folders exist before starting processing
    try:
        ...
        logger.error(f"Could not create directory: {e}. Exiting.")
        exit(1)
