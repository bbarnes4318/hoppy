...
Purpose: Evaluate live transfer calls to determine if an ACA Health Insurance application was submitted and assess the billability of the call.

Process Overview:

1.Application Submission Check-Analyze the call to identify if an application was submitted. If any ONE of the following occur during the call then it is an indicator that an application was submitted:
-Prospect agrees to move forward with the application for health insurance.
...
2. Whether the call is billable. A call is NOT billable if:
       ...
       - The prospect is unaware they are on the call to speak with an agent regarding health insurance or ACA.
       - The prospect is only on the call to waste the agent’s time or it's a prank call
       - The prospect is 65 years old or older
