...
def analyze_transcript_with_chatgpt(transcript):
    ...
    1. Whether the call is billable. A call is not billable if:
       - The prospect's combined credit card and/or personal loan debt is less than $8,000.
       - The prospect is already enrolled in another debt settlement program.
       - The prospect is in an active bankruptcy.
       - The prospect is unaware they are on the call to speak with an agent regarding their debt.
       - The prospect is only on the call to waste the agent’s time or it's a prank call.
    2. Whether the prospect's credit was pulled and if so, the total debt amount.
    3. Whether a sale was made
    4. The prospect's phone number
    5. Whether a follow-up call was set
    6. A brief call summary including key details and outcomes.
    ...
    response = openai.ChatCompletion.create(
        ...
        messages=[
            {"role": "system", "content": "You are a highly intelligent AI trained to analyze call transcripts for the deb settlement industry."},
