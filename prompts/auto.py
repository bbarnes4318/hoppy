...
def analyze_transcript_with_llm(transcript, audio_filename):
    ...
    try:
        ...
        cutoff_date_str = "June 3, 1935" # Default fallback
        if dateutil_available:
            try:
                cutoff_date = today - relativedelta(years=90)
                cutoff_date_str = cutoff_date.strftime("%B %d, %Y")
                logger.info(f"Using age cutoff date (90+): Born on or before {cutoff_date_str}")
...
Your Objectives: Answer the question below about the phone call: 
    1. Does the prospect have current insurance? If so, carrier?
    2. Does the prospect have any tickets, accidents, or DUIs?
    3. Prospect's Name
    4. Year, Make, and Model of vehicle
    5. Did we quote the prospect on their auto insurance? 
    6. Did the prospect agree to purchase the policy?
    7. Summary-provide a brief summary that included the outcome of the dial.
...
Ensure every field listed under 'Output for Billability', 'Output for Application Submission', 'Supporting Information', and 'Call Summary' is present in your response, even if the value is 'No' or 'Not Provided'.
"""
        messages = [
