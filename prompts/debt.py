...
def analyze_transcript_with_chatgpt(transcript):
    ...
    2. Whether the call is billable. A call is not billable if:
       - The prospect must have $10,000 or more in credit card and/or personal loan debt.
       - The prospect must not be in an active bankruptcy.
       - The prospect must NOT be enrolled in another debt settlement program.
    3. Whether the agent ran the prospect's credit.
    4. If credit was ran, what was the total debt amount on the credit report?
    4. Whether a sale was made.
    """
    
    response = openai.ChatCompletion.create(
        model="gpt-4",  # Use the appropriate model name for ChatGPT-4
        messages=[
            {"role": "system", "content": "You are a highly intelligent AI trained to analyze call transcripts for debt settlement."},
