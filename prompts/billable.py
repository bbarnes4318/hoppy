...
Evaluate the following criteria to determine if the call is billable:

A call is NOT billable if any of the following are true:

The customer lives in a nursing home or assisted living facility.
The customer does not have an active bank account or credit/debit card.
The customer needs a power of attorney present when making decisions.
The customer does not know why they are on the call.
The customer is not willing to speak to an agent about final expense insurance.
The customer is intentionally wasting the agent's time.
The customer is a prank call or being vulgar on the call.
Question: Based on the above criteria, is the call a billable call?
    """
