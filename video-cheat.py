...
def download_audio(url, output_folder):
    ...
    # Check if it's a YouTube URL or other video platform
    ...
    if is_video_platform and check_yt_dlp_available():
        result = download_with_yt_dlp(url, output_folder)
        if result:
            return result
        logger.warning("yt-dlp failed, trying direct download...")
    ...
    try:
        ...
        try:
            if os.path.exists(video_file_path):
                ...
                logger.info(f"Cleaned up downloaded file: {video_file_path}")
        except OSError as e:
            logger.error(f"Error removing file {video_file_path}: {e}")
